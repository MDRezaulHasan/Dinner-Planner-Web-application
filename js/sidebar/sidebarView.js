class SidebarView {
  ...
  render() {
    //Prie table in side bar
    h(
      "div",
      h(
        "div",
        this.model.getNumberOfGuests() <= 1
      ...
      h(
        ...
        h(
          ...
          h(
            "table",
            { class: "priceTable", border: "1" },
            h(
              "tr",
              h("th", { width: "350px" }, "Dish Name"),
              h("th", { width: "50px" }, "Dish Price")
            ),
            this.model
              .getMenu()
              .sort(
              ...
              .map(
                dish =>
                  h(
                    "tr",
                    { border: "1" },
                    h("td", { className: "td", width: "350px" }, dish.title),
                    h(
                      "td",
                      { width: "50px" },
                      (dish.dishPrice * this.model.getNumberOfGuests()).toFixed(
                        2
                      )
                    ),
                    h(
                      "button",
                      { onClick: event => this.onDeleteClicked(dish) },
                      "delete"
                    )
                  ) //parameter passed in sidebarController
              ),
            h(
              "tr",
              h("th", { width: "350px" }, " "),
              h(
                "th",
                { width: "50px" },
                "Total: " + model.getTotalPrice(this.model.getMenu()).toFixed(2)
              )
            )
          )
        )
